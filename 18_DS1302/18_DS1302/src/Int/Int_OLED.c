...
#define DEV_ADDR 0x78 // 从设备SSD1315设备地址(写操作)
#define DATA_MODE 0x40 //控制字节：连续数据
#define CMD_MODE 0X00 //控制字节：连续命令
...
void Int_OLED_Init()
{
    ...
    //开启屏幕
    cmd_bytes[5] = 0xAF;
    Int_OLED_WriteCommand(cmd_bytes,6);
}

void Int_OLED_Clean()
{
    u8 cmd_bytes[2]; //采用列寻址的方式
    u8 i,data_bytes[8] = {0}; //OLED低电平熄灭，循环使其清屏
    cmd_bytes[0] = 0x20;
    cmd_bytes[1] = 0x00;
    Int_OLED_WriteCommand(cmd_bytes,2);
    for ( i = 0; i < 128; i++)
    {
        Int_OLED_WriteDate(data_bytes,8);
    }
...
void Int_OLED_SetPointer(u8 page,u8 column)
{
    ...
    Int_OLED_WriteCommand(cmd_bytes,3);
}
...
void Int_OLED_ShowStr(u8 x, u8 y, u8 *str)
{
    ...
    //写入显示数据
    for ( i = 0; i < str[i] != 0 && i < 16 - x; i++)
    {
        Int_OLED_WriteDate(Int_OLED_Font[str[i] - 32],8);
    }
    ...
    //写入显示数据
     for ( i = 0; i < str[i] != 0 && i < 16 - x; i++)
    {
        Int_OLED_WriteDate(Int_OLED_Font[str[i] - 32] + 8,8);
    }
}
